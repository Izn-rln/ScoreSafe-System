    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            <div class="modal">
                ...
                <p>Are you sure you want to sign out? Any unsaved changes may be lost.</p>
...
async function safeFetchArray(url) {
    try {
        ...
        if (res.status === 429) { console.warn('Rate limited:', url); return []; }
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (avatarWrapper && picUrl && picUrl !== 'null') {
                let finalSrc = picUrl.startsWith('http')
                    ? picUrl.replace(/=s\d+-c/g, '=s400-c')
                    : `${API_BASE_URL}/uploads/${picUrl}`;
                avatarWrapper.innerHTML = `<img src="${finalSrc}" referrerpolicy="no-referrer"
...
async function saveProfileChanges(e) {
    ...
    if (!fullName || !email) { alert('Missing required fields.'); return; }
...
async function renderScoresTables() {
    ...
    try {
        ...
        records.forEach((r) => {
            ...
            const lockIcon   = r.is_finalized ? '<i class="fas fa-lock" style="color:#888;margin-left:5px;" title="Finalized"></i>' : '';
            const statusBadge = r.is_finalized
                ...
                : `<span class="badge" style="background:#f1f5f9;color:#475569;border:1px solid #e2e8f0;"><i class="fas fa-eye-slash"></i> DRAFT</span>`;

            tr.innerHTML = `
                <td>${esc(r.full_name) || 'Unknown'} ${lockIcon}</td>
                ...
                <td>
                    <div style="display:flex;gap:8px;justify-content:center;align-items:center;white-space:nowrap;">
                        <button class="btn" style="padding:6px 14px;font-size:.85rem;"
                            onclick="editScore(${r.id}, ${r.is_finalized})">
                            <i class="fas fa-pen"></i> Edit
                        </button>
                        <button class="btn outline" style="padding:6px 14px;font-size:.85rem;"
    ...
    try {
        ...
        if (approved.length === 0) {
            ...
            approved.forEach(f => {
                ...
                const tr = document.createElement('tr');
                tr.innerHTML = `
                    <td>${esc(f.email)} ${isSelf ? '<span style="font-size:.7rem;color:#888;">(you)</span>' : ''}</td>
                    <td>${esc(f.campus)}</td>
                    <td>
                        <span style="background:${f.is_admin ? '#fef3c7' : '#e0f2fe'};
                                     color:${f.is_admin ? '#92400e' : '#0369a1'};
                                     padding:4px 10px;border-radius:20px;font-size:.75rem;font-weight:bold;">
                    ...
                    <td style="text-align:center;">
                        <div style="display:flex;gap:8px;justify-content:center;align-items:center;">
                            ${isAdmin && !isSelf ? `
                                ${!f.is_admin ? `
                                    <button class="btn" style="padding:6px 14px;font-size:.85rem;"
                                        onclick="promoteToAdmin('${esc(f.email)}')">
                                        <i class="fas fa-shield-alt"></i> Promote
                                    </button>
                                ` : `
                                    <button class="btn outline" style="padding:6px 14px;font-size:.85rem;"
                                        onclick="demoteToTeacher('${esc(f.email)}')">
                                        <i class="fas fa-user"></i> Demote
                                    </button>
                                `}
                                <button class="btn danger" style="padding:6px 14px;font-size:.85rem;"
        ...
        if (isAdmin) renderPendingRequests(faculty.filter(f => f.is_pending));
...
async function renderStudentTable() {
    ...
    students.forEach(s => {
        ...
        tr.innerHTML = `
            ...
            <td><button class="btn outline" style="padding:6px 12px;font-size:.85rem;" onclick="handleRemoveStudent(${s.id},'${esc(s.full_name)}')">Remove</button></td>`;
        tbody.appendChild(tr);
    });
}
...
async function renderSubjectTable() {
    ...
    subjects.forEach(s => {
        ...
        tr.innerHTML = `
            <td>${esc(s.name)}</td>
            <td style="text-align:center;">
                <button class="btn danger" style="padding:6px 12px;font-size:.85rem;"
        ...
        section.innerHTML = `
            ...
            <div class="table-wrapper">
                <table id="pendingTable">
                    <thead><tr>
                        <th>Email</th><th>Campus</th><th>Requested By</th>
        ...
        <tr>
            ...
            <td style="text-align:center;">
                <div style="display:flex;gap:8px;justify-content:center;">
                    <button class="btn" style="padding:6px 14px;font-size:.85rem;"
                        onclick="approveFacultyRequest('${esc(p.email)}')">
                        <i class="fas fa-check"></i> Approve
                    </button>
                    <button class="btn danger" style="padding:6px 14px;font-size:.85rem;"
                        onclick="declineFacultyRequest('${esc(p.email)}')">
...
async function approveFacultyRequest(email) {
    ...
    const data = await res.json();
    res.ok ? (alert('Request approved!'), renderFacultyTable()) : alert(data.error);
}
...
async function declineFacultyRequest(email) {
    ...
    const data = await res.json();
    res.ok ? (alert('Request declined.'), renderFacultyTable()) : alert(data.error);
}
...
if (addTeacherForm) {
    addTeacherForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const data = await res.json();
            if (res.ok) { alert(data.message); addTeacherForm.reset(); renderFacultyTable(); }
...
if (recordForm) {
    recordForm.addEventListener('submit', async (e) => {
        ...
        if (!teacherId)      { alert('Teacher session not found. Please log in again.'); return; }
        ...
        if (isNaN(totalItems) || totalItems < 1) { alert('Please enter valid total items.'); return; }
        if (score > totalItems) { alert(`Score (${score}) cannot exceed Total Items (${totalItems}).`); scoreInput.focus(); return; }
        ...
        try {
            ...
            if (response.ok) {
                alert('Score recorded! Visible to student once finalized.');
                recordForm.reset();
...
if (authForm) {
    authForm.addEventListener('submit', async (e) => {
        ...
        const tempName = document.getElementById('authName').value.trim();
        const campus   = document.getElementById('authCampus')?.value || '';
        const me       = sessionStorage.getItem('username') || localStorage.getItem('username');

        if (email === me)        { alert('You cannot authorize yourself as a student.'); return; }
        ...
        try {
            ...
            const data = await res.json();
            if (res.ok) { alert(data.message || 'Student authorized!'); authForm.reset(); renderEnrollmentTable(); }
            else alert(data.error || 'Failed to authorize student.');
        } catch (err) { alert('Connection error.'); }
    });
}
...
if (addSubjectForm) {
    addSubjectForm.addEventListener('submit', async (e) => {
        ...
        if (!subjectName) { alert('Subject name is required.'); return; }
        try {
            ...
            const data = await res.json();
            if (res.ok) { alert('Subject added!'); addSubjectForm.reset(); renderSubjectTable(); }
...
async function lockScore(id, isLocked) {
    ...
    if (confirm('Finalize this record? This will publish the grade to the student.')) {
...
async function updateDashboardStats() {
    try {
        ...
        const approved = users.filter(u => u.role === 'student' && u.is_approved === 1);
        if (document.getElementById('totalRecords'))  document.getElementById('totalRecords').innerText  = scores.length;
        if (document.getElementById('totalStudents')) document.getElementById('totalStudents').innerText = approved.length;
...
async function loadDropdowns() {
    ...
    if (studentSelect) {
        ...
        studentSelect.innerHTML = '<option value="" disabled selected>-- Results --</option>' +
            activeStudents.map(s => `<option value="${s.id}">${esc(s.full_name)}</option>`).join('');
    }
    if (subjectSelect) {
        const subjects = await safeFetchArray(`${API_BASE_URL}/api/scores/get-subjects`);
...
async function renderEnrollmentTable() {
    ...
    tbody.innerHTML = pendingApproval.map(s => `
        <tr>
            <td><span class="status-badge ${s.is_verified ? 'active' : 'pending'}">${s.is_verified ? 'Pending Approval' : 'Authorized'}</span></td>
            <td>${esc(s.username)}</td>
            <td>${esc(s.full_name)}</td>
            <td>${esc(s.campus) || '---'}</td>
            <td>
                <div style="display:flex;gap:8px;align-items:center;">
                    <button class="btn" style="padding:6px 12px;font-size:.85rem;"
                        onclick="approveStudent(${s.id})"><i class="fas fa-check"></i> Approve</button>
                    <button class="btn danger" style="padding:6px 12px;font-size:.85rem;"
                        onclick="handleRemoveStudent(${parseInt(s.id)},'${esc(s.full_name)}')"><i class="fas fa-times"></i> Decline</button>
                </div>
            </td>
        </tr>`).join('');
}
    ...
    tbody.innerHTML = activeStudents.map(s => {
        ...
        return `
            <tr>
                ...
                <td style="text-align:center;">
                    <div style="display:flex;gap:8px;justify-content:center;align-items:center;">
                        <button class="btn outline" style="padding:6px 12px;font-size:.85rem;"
                            onclick="viewPerformance('${esc(s.username)}')">
                            <i class="fas fa-chart-bar"></i> View
                        </button>
                        <button class="btn danger" style="padding:6px 12px;font-size:.85rem;"
                            onclick="handleRemoveStudent(${s.id},'${esc(s.full_name)}')">
...
async function removeFaculty(email) {
    if (!confirm(`Remove ${email} from faculty?`)) return;
    const requesterEmail = sessionStorage.getItem('username') || localStorage.getItem('username');
    ...
    const data = await res.json();
    res.ok ? (alert('Faculty removed.'), renderFacultyTable()) : alert(data.error || 'Failed.');
}
...
async function promoteToAdmin(email) {
    ...
    const data = await res.json();
    res.ok ? (alert('Promoted!'), renderFacultyTable()) : alert(data.error || 'Failed.');
}
...
async function demoteToTeacher(email) {
    ...
    const data = await res.json();
    res.ok ? (alert('Demoted.'), renderFacultyTable()) : alert(data.error || 'Failed.');
}
...
async function viewPerformance(email) {
    try {
        ...
        if (finalized.length === 0) { alert(`No finalized records for:\n${email}`); return; }
    ...
    } catch { alert('Could not load performance data.'); }
}
...
function applyFilter(tableId, type, value) {
    ...
    table.querySelectorAll('tbody tr:not(.empty-row)').forEach(row => {
        if (type === 'reset' || value === 'all') { row.style.display = ''; return; }
        let text = '';
        if (type === 'campus')   text = row.children[1]?.innerText || '';
        if (type === 'role')     text = row.children[2]?.innerText || '';
        if (type === 'category') text = row.children[5]?.innerText || '';
        row.style.display = text.toLowerCase().includes(value.toLowerCase()) ? '' : 'none';
    });
    document.querySelectorAll('.filter-menu').forEach(m => m.classList.remove('active'));
}
...
document.querySelectorAll('.table-search').forEach(input => {
    input.addEventListener('input', function () {
        ...
        document.querySelectorAll(`${tableId} tbody tr:not(.empty-row)`).forEach(row => {
...
function setupSearchableDropdown(inputId, selectId) {
    ...
    input.addEventListener('input', function () {
        ...
        Array.from(select.options).forEach(opt => {
            ...
            const m = opt.text.toLowerCase().includes(filter);
            opt.style.display = m ? '' : 'none';
            if (m) any = true;
        });
        select.style.display = any ? 'block' : 'none';
    });

    select.addEventListener('change', function () {
        const sel = this.options[this.selectedIndex];
        if (sel?.value) { input.value = sel.text; select.style.display = 'none'; }
    });
...
window.addEventListener('load', () => {
    if (document.getElementById('teacherProfileForm')) {
        displayProfileInfo();
        document.getElementById('teacherProfileForm').addEventListener('submit', saveProfileChanges);
        document.getElementById('resetTeacherProfile')?.addEventListener('click', () => {
            if (confirm('Discard all unsaved changes?')) {
                displayProfileInfo();
                const fi = document.getElementById('teacherAvatar');
                if (fi) fi.value = '';
            }
        });
    }
    ...
    setTimeout(() => { if (document.getElementById('scoresTable'))    renderScoresTables(); },       400);
    setTimeout(() => { if (document.getElementById('studentsTable'))  renderActiveStudentTable(); }, 600);
    setTimeout(() => { if (document.getElementById('subjectsTable'))  renderSubjectTable(); },       800);
    setTimeout(() => { if (document.getElementById('facultyTable'))   renderFacultyTable(); },      1000);
    setTimeout(() => { if (document.getElementById('enrollmentTable')) renderEnrollmentTable(); },  1200);
});
